...
# Combine train and test for 80-20 split
...
X_train, X_val, y_train, y_val, seq_len_train, seq_len_val = train_test_split(
    X, y, seq_len, test_size=0.2, random_state=42)
...
print(f'Average Train Loss: {train_losses[-1]:.4f}')
print(f'Average Val Loss: {val_losses[-1]:.4f}')
print(f'Average MAE (Train): {mae_train_scores[-1]:.4f}')
print(f'Average MSE (Train): {mse_train_scores[-1]:.4f}')
print(f'Average R² (Train): {r2_train_scores[-1]:.4f}')
print(f'Average Correlation coefficient (r) (Train): {r_train_scores[-1]:.4f}')
print(f'Average MAE (Val): {mae_val_scores[-1]:.4f}')
print(f'Average MSE (Val): {mse_val_scores[-1]:.4f}')
print(f'Average R² (Val): {r2_val_scores[-1]:.4f}')
print(f'Average Correlation coefficient (r) (Val): {r_val_scores[-1]:.4f}')

# Save the trained model
torch.save(model.state_dict(), 'gru_model_no_cv.pth')
...
plt.ylabel('Loss')
plt.legend()
plt.title('Loss Curves without Cross-Validation')
plt.show()

# Plot MAE over epochs
plt.figure(figsize=(10, 5))
plt.plot(range(num_epochs), mae_train_scores, label='Train MAE')
plt.plot(range(num_epochs), mae_val_scores, label='Validation MAE')
plt.xlabel('Epochs')
plt.ylabel('MAE')
plt.legend()
plt.title('MAE Over Epochs without Cross-Validation')
plt.show()

# Plot R² over epochs
plt.figure(figsize=(10, 5))
plt.plot(range(num_epochs), r2_train_scores, label='Train R²')
plt.plot(range(num_epochs), r2_val_scores, label='Validation R²')
plt.xlabel('Epochs')
plt.ylabel('R²')
plt.legend()
plt.title('R² Over Epochs without Cross-Validation')
plt.show()
