...
if not os.path.exists('./szc/Atext') or not os.path.exists('./szc/Models/Atext'):
    sys.exit()
...
for test_file in list_of_files:
    ...
    for file in train_files:
        ...
        if os.path.exists(file):
            ...
            train_data = train_data[train_data['MAPPSZ'] != '.']
            ...
            train_labels = train_data['MAPPSZ'].astype(float).tolist()
            #train_labels = [x * 7 for x in train_labels]
    ...
    if os.path.exists(test_file):
        ...
        # Preprocess and standardize labels
        test_data = test_data[test_data['MAPPSZ'] != '.']
        # scaler = StandardScaler()
        ...
        test_labels = test_data['MAPPSZ'].astype(float).tolist()
        #test_labels = [x * 7 for x in test_labels]
    ...
    save_directory = './szc/Models/Atext/'+str(re.findall(r'\d', str(test_file))[0])+'/'
    ...
    model_save_path = './szc/Atext/'+str(re.findall(r'\d', str(test_file))[0])+'_'+str(patience)+'/'
