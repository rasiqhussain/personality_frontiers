...
if not os.path.exists('./occ/Atext') or not os.path.exists('./occ/Models/Atext'):
    sys.exit()
...
for test_file in list_of_files:
    ...
    for file in train_files:
        ...
        if os.path.exists(file):
            ...
            train_data = train_data[train_data['MAPPOC'] != '.']
            ...
            train_labels = train_data['MAPPOC'].astype(float).tolist()
            #train_labels = [x * 8 for x in train_labels]
    ...
    if os.path.exists(test_file):
        ...
        # Preprocess and standardize labels
        test_data = test_data[test_data['MAPPOC'] != '.']
        # scaler = StandardScaler()
        ...
        test_labels = test_data['MAPPOC'].astype(float).tolist()
        #test_labels = [x * 8 for x in test_labels]
    ...
    save_directory = './occ/Models/Atext/'+str(re.findall(r'\d', str(test_file))[0])+'/'
    ...
    model_save_path = './occ/Atext/'+str(re.findall(r'\d', str(test_file))[0])+'_'+str(patience)+'/'
